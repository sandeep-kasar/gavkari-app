...
import com.gavkariapp.helper.SelectImageForSaleFragment
...
import com.gavkariapp.service.LocationService
...
import com.gavkariapp.constant.AppConstant.UNDER_REVIEW as it


open class CreateSellAdActivity : BaseActivity(), View.OnClickListener,
    ...
    private lateinit var buySale: BuySaleType
    private lateinit var buySaleObj: SaleCreateBody
    private lateinit var selectImageFragment: SelectImageForSaleFragment
    ...
    private fun setUpView() {
        ...
        checkLocationPermissionAutodetect()

        selectImageFragment = SelectImageForSaleFragment(this)
        hashMap = HashMap()
        ...
        buySale = intent.getSerializableExtra("buySaleType") as BuySaleType

        buySaleObj = SaleCreateBody("","","", it,0,0,"","","",
                "","","","","","","","","",
                "","","","","","","","","","",
                "","","",saleMediaList)
        ...
        if(buySale!=null){
            ...
            when {
                buySale.tab_type == ANIMAL -> {
                    ...
                    setupToolbar(R.id.toolbarHome, getString(R.string.lbl_sale_animal))
                ...
                buySale.tab_type == MACHINERY -> {
                    setupToolbar(R.id.toolbarHome, getString(R.string.lbl_sale_machinery))
                    layAnimal.visibility = View.GONE
                ...
                buySale.tab_type == EQUIPMENTS -> {
                    ...
                    setupToolbar(R.id.toolbarHome, getString(R.string.lbl_sale_equipment))
                    layAnimal.visibility = View.GONE
                    layMachine.visibility = View.GONE
    ...
    private fun setSpinner(type:Int){
        ...
        val adapter = ArrayAdapter(this,android.R.layout.simple_spinner_dropdown_item, breedList)
        spnBreed.adapter = adapter
        spnBreed.onItemSelectedListener = object :AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>,view: View, position: Int, id: Long) {
                //Toast.makeText(this@CreateSellAdActivity,breedList[position], Toast.LENGTH_SHORT).show()
        ...
        spnPregCount.onItemSelectedListener = object :AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>,view: View, position: Int, id: Long) {
                //Toast.makeText(this@CreateSellAdActivity,pregCount[position], Toast.LENGTH_SHORT).show()
                when {
                    position > 1 -> {
        ...
        spnPregStatus.onItemSelectedListener = object :AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>,view: View, position: Int, id: Long) {
                //Toast.makeText(this@CreateSellAdActivity,pregStatus[position], Toast.LENGTH_SHORT).show()
                if(position>0){
    ...
    private fun checkLocationPermissionAutodetect() {
        Dexter.withActivity(this)
                .withPermission(android.Manifest.permission.ACCESS_FINE_LOCATION)
                .withListener(object : PermissionListener {
                    override fun onPermissionGranted(response: PermissionGrantedResponse) {
    ...
    private fun getAddress(){
        ...
        var latitude = prefs[ApiConstant.LATITUDE, "-1"]!!
        var longitude = prefs[ApiConstant.LONGITUDE, "-1"]!!
        ...
        edtMapLocation.setText(address)
    }
    ...
    private fun showSettingsDialog() {
        showAlert(this@CreateSellAdActivity, this@CreateSellAdActivity,
    ...
    fun cameraIntent() {
        ...
        fileUri = Util.getOutputMediaFileUri(this@CreateSellAdActivity, IMAGE_DIRECTORY_NAME)
    ...
    private fun continueTwo() {
        ...
        when{
            ...
            buySale.tab_type == EQUIPMENTS -> {
                buySaleObj.company = edtCompanyEq.text.toString().trim()
                buySaleObj.year = edtBuyYearEq.text.toString().trim()
                buySaleObj.power = edtPowerEq.text.toString().trim()
                buySaleObj.capacity = edtCapacityEq.text.toString().trim()
        ...
        if (isTrue) {
            ...
            if (isTrueType) {
                ...
                if (isTrueLast) {
                    ...
                    //Toast.makeText(this@CreateSellAdActivity, buySale.name + buySale.tab_type + buySale.type, Toast.LENGTH_SHORT).show()
    ...
    private fun createSale(saleCreateBody: SaleCreateBody) {
        ...
        ApiClient.get().create(ApiInterface::class.java)
                .createBuySaleAd(saleCreateBody)
                    ...
                    override fun onResponse(call: Call<BuySaleResponse>?, response: Response<BuySaleResponse>?) {
                        if (response!!.code() == 200) {
                            if (response.body()?.status == HttpConstant.SUCCESS) {
                                runOnUiThread {
                                    dismissProgress()
                                    showSuccess(getString(R.string.msg_ad_success))
                                    finish()
                                    startActivity(Intent(applicationContext, SuccessActivity::class.java)
                                            .putExtra("screen_type","saleAd"))
    ...
    private fun setImage(filePath: String) {
        val imageSetAsyncTask = ImageSetAsyncTask(this@CreateSellAdActivity,
                this@CreateSellAdActivity, IMAGE_SELECTED)
                ...
                .enqueue(object : Callback<UploadFile> {
                    ...
                    override fun onFailure(call: Call<UploadFile>?, t: Throwable?) {
                        ...
                        runOnUiThread {
                            ...
                            showSuccess(getString(R.string.warning_try_later))
                        }
                    }
                })
