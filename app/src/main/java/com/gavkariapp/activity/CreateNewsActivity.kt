...
import com.gavkariapp.helper.SelectImageFragmentNews
...
open class CreateNewsActivity : BaseActivity(), View.OnClickListener,ImageSetAsyncTask.ImageCallback {


    private val IMAGE_DIRECTORY_NAME = "GavkariApp"
    private val REQUEST_IMAGE_CAPTURE = 3
    private val REQUEST_IMAGE_FROM_GALLERY = 4
    private lateinit var selectImageFragment: SelectImageFragmentNews
    private lateinit var fileUri: Uri
    private var IMAGE_SELECTED = IMAGE_NOT_SELECTED
    private lateinit var hashMap: HashMap<String, String>
    private lateinit var newsMatter: NewsMatter
    ...
    private fun setUpView() {
        ...
        //init
        selectImageFragment = SelectImageFragmentNews(this)
        hashMap = HashMap()
        ...
        newsMatter = intent.getSerializableExtra("newsMatter") as NewsMatter

        if(newsMatter!=null){
            edtNewsTitle.setText(newsMatter.title)
            edtNewsTitle.hint = newsMatter.title
            edtNewsSource.setText(newsMatter.source)
            edtNewsSource.hint = newsMatter.source
            edtNewsDescription.setText(newsMatter.description)
            edtNewsDescription.hint = newsMatter.description
        }
    ...
    fun cameraIntent() {
        ...
        fileUri = Util.getOutputMediaFileUri(this@CreateNewsActivity, IMAGE_DIRECTORY_NAME)
    ...
    private fun continueTwo() {
        ...
        if (isTrue) {
            ...
            val createNewsBody = CreateNewsBody("0",useId!!, villageId!!,"0","0",newsMatter.type,
                    ...
                    override fun onResponse(call: Call<CommonResponse>?, response: Response<CommonResponse>?) {
                        if (response!!.code() == 200) {
                            if (response.body()?.status == HttpConstant.SUCCESS) {
                                runOnUiThread {
                                    dismissProgress()
                                    startActivity(Intent(this@CreateNewsActivity,SuccessActivity::class.java).
                                            putExtra("screen_type","news"))
                                    ActivityCompat.finishAffinity(this@CreateNewsActivity)
                                }
    ...
    private fun setImage(filePath: String) {
        val imageSetAsyncTask = ImageSetAsyncTask(this@CreateNewsActivity,
                this@CreateNewsActivity, IMAGE_SELECTED)
